...
function BarrowCont({ barrow, setbarrow, barrowInfo, memdata, navigate, id }) {
  ...
  let memform = useFormik({
    ...
    onSubmit: (values) => {
      putBookData(barrowInfo, id, values.memId)
    }
  })
  ...
  // console.log(barrowInfo, id);

  let test = () => {
    console.log("sucess");
  }
  let putBookData = ((arr, add, ele) => {
    // console.log(arr, ele);
    arr.map((val) => {
      if (val == ele) {
    ...
    {
      setbarrow ? fetch(`https://6397271477359127a02e817e.mockapi.io/books/${add}`, {
        method: "PUT",
        ...
        // alert("Done");
        // console.log("Done");
        putUserData(ele, memdata, add)

      }) : alert("This id already barrowed")
    }
  })
  function putUserData(id, arr, ele) {
    console.log("Running");
    arr.map(val => {
      if (val.id == id) {
        fetch(`https://6397271477359127a02e817e.mockapi.io/memebers/${id}`, {
          method: "PUT",
          body: JSON.stringify({ barrowed: [...val.barrowed, parseInt(ele)] }),
          headers: {
            "Content-type": "application/json; charset=UTF-8"
          }
        }).then(() => {
